...
class GetAvgFruitNVeggieInteractorTest {
    ...
    fun should_ReturnLowest10CityPricesComparedToSalaries_When_TheListNotEmptyAndNotNullAndTheAttributesNotNull(){
        ...
        val dataSource: CostOfLivingDataSource = CsvDataSource(csvParser, GetAvgFruitNVeggieInteractorTest.SUB_FILE_NAME) // CSV : comma separated values
        ...
        //then find the lowest 10 cities fruits and vegetables compared to salaries
            assertEquals(list.subList(0, 9), cheapestCityInPrices)
    }
    @Test
    fun should_ReturnFirst10City_When_just10CityProvided() {
        //given List of 10 cities prices of fruit and vegetables and salaries
        val csvParser = CsvParser()
        val dataSource: CostOfLivingDataSource = CsvDataSource(csvParser, GetAvgFruitNVeggieInteractorTest.TEN_CITY_FILE)
    ...
    fun should_ReturnNull_When_TheFileIsEmpty(){
        //given
        val csvParser = CsvParser()
        val dataSource: CostOfLivingDataSource = CsvDataSource(csvParser, GetAvgFruitNVeggieInteractorTest.EMPTY_FILE)
        cheapestFruitNVeggie=GetAvgFruitNVeggieInteractor(dataSource)
    ...
    fun should_ReturnNull_When_AllTheSalariesAreNull(){
        val csvParser = CsvParser()
        val dataSource: CostOfLivingDataSource = CsvDataSource(csvParser, GetAvgFruitNVeggieInteractorTest.NULL_SALARIES_FILE)
        cheapestFruitNVeggie=GetAvgFruitNVeggieInteractor(dataSource)
    ...
    fun should_ReturnNull_When_AllTheFruitsAndVegetablesPricesAreNull(){
        //given
       val csvParser = CsvParser()
        val dataSource: CostOfLivingDataSource = CsvDataSource(csvParser, GetAvgFruitNVeggieInteractorTest.NULL_FRUITNVEGGIE_COST_FILE)
        cheapestFruitNVeggie=GetAvgFruitNVeggieInteractor(dataSource)
